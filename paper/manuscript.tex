...
A single submission companion archive, \texttt{OBS\_Mechanism\_Submission\_Companion\_v0.2}, contains an index and the authoritative exact packages for the 12-state theta witness, the corrected spectral analysis, the causal-window proof, and the Lean symbolic companion. The exact rate tables, stationary data, characteristic polynomials, Arb/Acb and rational Rouch\'e certificates, independent spectral audits, structural diagnostics, hostile reconstructions, build transcripts, and SHA-256 manifests are included. Each constituent package has pinned dependencies and one-command or explicitly enumerated reproduction instructions. The validated numerical certificates require Python and \texttt{python-flint}. The Lean component is \texttt{OBS\_Lean\_Core\_Closed\_Package\_v0.2}, pinned to Lean 4.29.0 and a fixed Mathlib revision. Its umbrella build includes \texttt{rowMarkov\_paperCore}, \texttt{rowMarkov\_etaCur\_eq\_eta\_iff}, the matched-subdivision modules, and the exact local-obstruction module. Lean does not verify the witness-specific spectrum, logarithmic interval evaluation, or causal-window root isolation; those remain independent exact and validated-computation claims. A separate manuscript source package contains the LaTeX source, bibliography, figures, and theorem-to-manuscript crosswalk.
